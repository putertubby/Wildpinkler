    ...
    private async Task<string> BuildSystemPromptAsync(CancellationToken cancellationToken)
    {
        ...
        else
        {
            ...
            [
                ...
                "- To enable, disable or reorder a mod or tool in a profile, call profiles.getLoadOrder first",
                "  to get real folder ids, then profiles.setModEnabled, profiles.setToolEnabled or",
                "  profiles.reorderMod. Report only what those actions actually returned, never assume one",
                "  succeeded from its arguments alone.",
                "- To see what a mod actually depends on, call mods.getDependencies - do not infer this from",
                "  profiles.diagnoseDependencies, which only reports problems among a profile's enabled mods,",
                "  not the full set of declared dependencies. To add, remove or change a dependency edge, call",
                "  mods.addDependency, mods.removeDependency or mods.updateDependency (enable the 'mods' group",
                "  first if it is not yet available); these need the user's approval like any other change.",
                ...
                "- Before answering any question about load-order correctness, missing/disabled/conflicting",
                "  dependencies, or 'known issues' with a profile, call profiles.diagnoseDependencies and base",
                "  the answer only on what it returns. Do not guess at compatibility from memory.",
                string.Empty,
                "Safety:",
