...
/// </summary>
public sealed class RemoteMetadataService
{
    private readonly RemoteSiteRegistry _registry;
    private readonly RemoteSiteContext _context;

    public RemoteMetadataService(RemoteSiteRegistry registry, RemoteSiteContext context)
    {
        _registry = registry;
        _context = context;
    }
    ...
    // MD5 is not a security control here: it is the hash Nexus publishes for file identification.
    [SuppressMessage("Security", "CA5351:Do not use broken cryptographic algorithms", Justification = "Fixed by the remote site's file-lookup API; used for identification, not integrity against an attacker.")]
