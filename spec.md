...
- Definitions are imported and exported from the Games panel. A definition file is untrusted input and is validated on read: the id must match `^[a-z0-9][a-z0-9._-]{0,63}$`, every relative-path field must be relative and resolve inside the install folder (no rooted, UNC, `..` or drive-qualified paths), variable names must match `^[A-Za-z][A-Za-z0-9_]{0,31}$`, merged-view names must be such an identifier or a `${identifier}` reference, and names must be unique within their own collection. Variable names are case-sensitive end to end, so a local name such as `localappdata` is distinct from the app-predefined system variable `LocalAppData`; the only local names that are rejected are those that match a global (system or profile built-in) name case-exactly, each reported with the message `'{name}' is a reserved global variable name and cannot be redefined by a local variable. Variable names are case-sensitive; choose a different name.` A definition's lowercase derived variables (`documents`, `localappdata`) built on top of the system variables are the sanctioned pattern. A merged view must declare at least one branch, and file size, variable, marker and merged-view/branch counts are capped. Suggested launch arguments are presented to the user for confirmation and never applied silently.
...
- Exporting a target's merged views bakes every local/user variable in fully, but a built-in variable (a system folder, or one of the profile's own computed built-ins) is never baked in as literal text - the resolved absolute path is symbolized back into its `${name}` placeholder wherever the variable's resolved value is a full path-segment prefix of the branch or mount path (the path equals the value, or a directory separator follows it - so `…\AppData\Local` never symbolizes a `…\AppData\LocalLow` path), with the remaining path always kept after the placeholder. Computed built-ins are resolvable through that same target's exported `variables` map; system folders are resolved natively by uufs64 and therefore stay out of the map. This keeps the exported config valid even if the profile folder or install path later moves.
...
- `workingDirectory` describes the launched executable's identity as seen through the virtual file system - resolved against the mounted install root, never against wherever the executable actually sits on disk. For the ordinary game executable and every tool this is identical to the real path, but a mod designated as the game's launcher (see [Mods as game launchers](#mods-as-game-launchers)) physically lives inside that mod's own installed folder; once its branch is mounted at the game's install root, the exported `workingDirectory` reflects where the game sees it there instead. Wildpinkler passes that same virtual path to the loader via `--target` (with its directory via `--curdir`, plus `--args` and `--steamid` when non-empty) rather than through the config, having first validated that the real on-disk executable exists.
- Launching exports the target's configuration and then starts `uufs64ldr.exe --target <virtual executable path> --curdir <directory of the virtual executable> [--args <arguments>] [--steamid <id>] <configPath>`. The loader process itself is not given an explicit working directory; its working directory is conveyed to the loader through the `--curdir` argument, which is always the directory of the target's virtual executable. The target's real on-disk executable is validated to exist first; the loader receives the virtual path, which it resolves through the injected file system.
...
- The descriptor (schema 5 of the game definition) names the plugin file extensions
  (`.esm`, `.esp`, `.esl`), the data folder they live in relative to the install folder (`Data`),
  and the **full path to the list file** (`listPath`). The path may use profile variables —
  for Skyrim SE this is `${localappdata}\plugins.txt`. There is no separate list file name or
  view variable. A tool definition's optional `sortsPluginList` flag marks tools that can
  reorder the plugin list (LOOT is the built-in example).
- When writing, the expanded `listPath` is matched against the profile's views: the first view
  whose mount path is a prefix of the real path (segment-aware, case-insensitive match starting
  at the root; where several views match, the deepest mount wins) is used, and the path
  remaining after removing the mount path is combined with that view's writable branch 0 (the
  top branch). The list therefore lives in the profile's own writable branch — never into the
  game directory and never shared between profiles or other mod managers — so each profile keeps
  its own order with no collisions.
- When a game target is launched, the list is regenerated together with the `profile.json` export,
  ...
  - Only the `<data folder>` of installed mods is scanned, in load-order priority — the game's
    own installation directory is never scanned, so no plugin exclusion list is needed. A plugin
    file name claimed by a higher-priority mod shadows the same name in lower ones.
  - Every mod's plugins are written, enabled and disabled alike: the enabled state is reflected
    by prefixing each data file belonging to an enabled mod with an asterisk (`*`). Because
    disabling a mod only removes its asterisk rather than its entries, toggling enabled state
    never makes the load order stale.
